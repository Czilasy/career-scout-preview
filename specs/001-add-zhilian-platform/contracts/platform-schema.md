...
最近结果加载使用结果自身返回的 `platform`，不触发新任务草稿切换。

> 首屏例外：页面第一次拿到最近结果时，允许把新任务草稿平台一次性对齐到该结果平台（仅此一次），避免顶部平台滑块与品牌色分属不同平台、需要用户手动切一次才一致。此后（含任务暂停/完成后再次加载结果）一律不再改写草稿平台。对齐必须同时重新加载该平台的 schema 与城市目录，不得带着旧平台的 `filter_schema_version` 提交。

反过来，用户切换草稿平台也不重新标记已展示结果。
