...
**Scale/Scope**: 10 个速度字段（含 JD 并发 Tab 数）、4 个执行阶段、3 个任务规模、3 个系统模式、1 个最近自定义配置；最终最多 9 个内部模式槽位，但允许多个槽位引用同一配置
...
**Changes**:
- 新增 `execution_config.py`，定义速度字段（含 JD 并发 Tab 数）、类型、物理边界、规范序列化和摘要。
...
### Slice 4A - 诊断证据与唯一阻断纠偏

**Goal**: 修正真实实验暴露的测量缺口，使后台保留全部安全诊断事实，而控制层和前端只归并、展示一个最终硬错误。

**Changes**:
- 为 AI 调用建立 attempt 级关联标识和结构化诊断字段，区分连接、TLS、HTTP、流读取、空响应、截断、非 JSON 与 JSON 解析失败；敏感原文不得进入通用事件。
- 记录每次失败、等待、重试决定和最终结果；需要保留失败响应时写入本地受限、脱敏诊断产物并通过摘要引用。
- 将 `item_terminal` 延迟到全部允许重试结束后，每个输入索引只发射一次最终终态。
- 控制层按确定性优先级归并唯一硬错误并原子阻断，前端只展示该错误和关联标识，后台事件链不被裁剪。
- 拒绝零输入空跑、重复终态、缺失终态和索引错配轮次。

**Gate**: 确定性故障注入能从持久化事件区分所有规定失败阶段；首次失败后恢复时终态唯一且不阻断，重试耗尽时后台保留完整链而前端只有一个硬错误；零输入轮次不可确认。
