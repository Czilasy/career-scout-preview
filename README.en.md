...
8. **Dual browser accounts A/B**: Advanced execution settings provide Account A and Account B. A uses the default isolated profile `~/.career-scout/chrome-profile`; B uses the project-local `.chrome-profiles/account_b`. The selected account is frozen when a task is created; start, resume, recrawl, cancel, and finish all activate the matching profile and replace a Chrome running the wrong account on the CDP port. Switching is locked while a task is running or paused, so choose the account for the next flow after the current task is closed.
...
WebUI advanced execution settings also support Account B, using the project-local `.chrome-profiles/account_b`. Account B is a completely separate isolated profile that requires its own manual BOSS login on first use; the CLI `--setup-chrome` still manages Account A's default profile.
