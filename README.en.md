...
WebUI advanced execution settings also support Account B, using the project-local `.chrome-profiles/account_b`. Account B is a completely separate isolated profile that requires its own manual BOSS login on first use; the CLI `--setup-chrome` still manages Account A's default profile. Account switching only closes Chrome using one of the two known profiles; an unknown Chrome owning the CDP port is never closed automatically.
