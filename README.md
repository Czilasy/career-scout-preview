...
浏览器访问 `http://127.0.0.1:5000`。根路径 `/` 是唯一正式前端入口，仓库不再保留平行版本页面。工作台是一个深色求职工作台：左侧可折叠设置区用于画像、简历和 AI 设置，主区为单列固定高度的大岗位卡片流。

### 核心能力

1. **简历解析**：上传 TXT / PDF / DOCX 简历后，AI 解析岗位方向、城市、技能与最多 3 组搜索关键词。上传按钮会原位显示分析中、成功或失败重试状态；未勾选 AI 同意时只在本地保存并明确标记“未分析”，不会创建永久排队任务。用户可手动补充或覆盖，**手动条件永远优先**。手动方向支持分别填写方向名称与 1–3 个搜索词，也兼容在方向框中用中英文逗号一次输入并自动拆分。
