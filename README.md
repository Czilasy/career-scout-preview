# Career Scout v1.10.0 · BOSS直聘 & 智联招聘职位助手
...
- 最新正式版：v1.10.0
- Windows 安装包：`CareerScout-v1.10.0.exe`，下载后双击运行
- macOS 安装包：`CareerScout-v1.10.0.dmg`，首次打开见下方 Gatekeeper 说明
...
- **窗口记忆**：首次启动直接最大化打开；窗口四边和四角都能拖动调整大小，调整后的大小与位置会被记住（最大化关闭不丢失），任何情况下窗口都不会超出屏幕。
- **自绘标题栏（Windows）**：Windows 桌面版使用与主题融合的自绘标题栏，无系统白条；标题栏支持按住拖拽移动、拖到屏幕顶端最大化、最大化后向下拖拽还原、双击最大化/还原，右上角为最小化 / 最大化-还原 / 关闭按钮（macOS 保持系统原生标题栏）。
...
- **放弃本轮**：步骤 2 或步骤 3 可放弃当前轮次，系统停止任务并清空当前现场回到上传页；已经保存的旧轮次仍可从历史轮次查看，步骤 3 的详情补抓则只停止补抓并保留当前结果。
...
- **智联稳定性**：详情页 Chrome 错误页只记为单条失败，不再误判平台封禁；暂停的智联筛选继续时会恢复原流程冻结的账号与浏览器身份，待确认重抓也会沿用该身份，身份不足时明确阻断并回到结果页，不留下无法操作的任务。
...
- **完成状态有证据才算数**：任务会明确区分完整成功、空结果、部分完成、失败、无法确认和中断；“无法确认”会建议重新执行，历史数据不会被悄悄改写为新结论。
