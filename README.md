...
续跑成功后旧中断记录不会再次弹出；同一来源的重复续跑会被拒绝。
...
JD 正文本身包含“限流/解锁”等岗位职责词时不会被误判成账号限流页，只有拿不到真实 JD 的页面才做风控分类。
...
重抓期间页面会按批实时合并已完成判定与 JD，暂停后刷新仍展示来源 run 的最新结果，而不是旧快照；重抓中途“结束并保存”时，不匹配原因、caveats 和已筛除岗位都会保留，不再出现原因空白或“已筛除 0”。
服务重启中断的 AI 筛选同样可以直接结束保存；刷新后恢复中断任务时会保留上次筛选条件与画像，旧历史结果不会覆盖中断提示。
完成任务后的计数同样按保留岗位统计，粗筛剔除不再显示成“未开始”；已有更新的完成结果时，旧暂停任务不会抢占刷新后的恢复提示。
