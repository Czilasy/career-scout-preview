...
抓完直接拿到：薪资分布、经验要求、高频技能词、求职材料优化提示词。CLI 提示词只基于岗位数据，不读取本地简历文件。可选 WebUI 会根据你手工填写的目标画像做本地硬筛选和可解释排序，但不会读取简历或判断录用概率。
...
浏览器访问 `http://127.0.0.1:5000`。工作台提供：

- 持久任务状态和日志，刷新页面后仍能恢复；支持取消和重试，服务重启会把未完成任务明确标记为中断。
- 每次任务使用独立列表/详情文件，不会误读其他任务或“最新 JSON”。
- 筛选项直接来自抓取核心的统一映射，避免页面标签和值错配。
- 目标岗位、必备/加分技能、排除词、地区、最低薪资和公司黑名单。
- 程序执行地点、薪资、排除词等硬条件淘汰；技能用于排序，并展示命中、缺失和风险原因。
- 当前任务的市场摘要和 CSV 下载。

状态数据库保存在 `~/.career-scout/webui/webui.db`，岗位结果仍保存在 `~/.career-scout/job-result/`。如需让 WebUI 使用指定 Python，可在启动前设置 `BOSS_PYTHON` 环境变量。
