...
WebUI 高级执行设置还支持账号 B，对应项目根目录的 `.chrome-profiles/account_b`。账号 B 与账号 A 是完全独立的隔离 profile，首次选择账号 B 后需要在该 Chrome 中单独登录；命令行 `--setup-chrome` 仍固定管理账号 A 的默认 profile。账号切换只关闭 A/B 两个已知 profile 对应的 Chrome；端口被其他 Chrome 占用时不会自动关闭。
