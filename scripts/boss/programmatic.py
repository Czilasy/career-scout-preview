...
def run_search_programmatic(
    *,
    ...
    # 只在真正进入执行上下文后置位，避免早期校验失败把 _facade()._run_active 永久留 True。
    _facade()._run_active = True
    with redirect:
        try:
            # 登录状态检测
            ...
            if not skip_login_check:
                print("检测登录状态...")
                if not _facade().check_login_state(cdp_port):
            ...
            if analysis:
                ...
                _facade().analyze(list_data, details, search_keyword=keyword)
