...
def clean_skill_tag(tag):
    ...
    schedule_or_duration = (
        re.fullmatch(r"\d+\s*天\s*/\s*周", tag)
        or re.fullmatch(r"\d+\s*个?月(?:以上)?", tag)
        or tag in {"实习", "兼职", "全职", "短期实习", "长期实习"}
    )
    return "" if tag in noise or schedule_or_duration else tag
