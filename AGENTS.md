...
1. **只写用户能感觉到的事**：写"用户得到什么"（增加/优化/修复后的体验），不写"怎么做到的"（不写测试、代码结构、内部重构、技术细节）。
2. **每条一句话，一句话一个变化**；全版按用户价值排序，3~6 条封顶，凑数的不要。
3. **类别词表**（每条开头用其一）：
   - `增加`：多了用户能用的新东西
   - `优化`：用起来更顺、更快、更好看
   - `修复`：之前坏了/不对的事，现在好了
4. **素材平时攒、发布时取**：开发/还债每完成一件用户可感知的事，随手以本口径记进 CHANGELOG；发布时直接读对应版本条目填入 GitHub Release，不现场编。
5. **正反例**：
   - 反例（太简）：`修复：常规修复`；反例（太技术）：`重构 filterGroups 派生逻辑，新增 12 种旋转动效`。
   - 正例：`增加：全新的万花筒彩蛋主题，可长按顶部入口切换`；`优化：浏览器与账号入口合并，切换账号更顺手`；`修复：某些情况下任务状态显示不准的问题`。
