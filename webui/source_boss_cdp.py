...
from webui.source_boss_helpers import _classify_failed_code, _exit_reason, _input_hash, _normalize_job_fields, _record_success_signal, _safe_host, _safe_tail, SCRAPER_FILTER_FIELDS
...
class BossCdpSource(_BossCdpDetailMixin):
    ...
    def __init__(self, *, python_executable: str | None=None, cwd: Path | None=None, scraper_path: Path | None=None, env: dict | None=None, timeout_seconds: int=600, runner: Callable[..., subprocess.Popen] | None=None, executor: ScraperExecutor | None=None, cancel_event=None, artifact_root: Path | str | None=None, max_artifact_bytes: int=20000000, cdp_port: int=boss.DEFAULT_CDP_PORT, breaker: SourceCircuitBreaker | None=None, browser_account: str | None=None, in_process: bool=False, run_id: str=''):
        ...
        self.env = env or {'PYTHONUTF8': '1', 'PYTHONIOENCODING': 'utf-8', 'PYTHONUNBUFFERED': '1', 'CAREER_SCOUT_LOG_LEVEL': 'INFO', **os.environ}
    ...
    def preflight(self) -> SourceOutcome:
        ...
        if boss.requests is None:
            return SourceOutcome.failure(failed_code='source_unreachable', safe_log='runtime_dependency_missing')
        try:
            response = boss.requests.get(f'http://127.0.0.1:{self.cdp_port}/json/version', timeout=5)
            if response.status_code != 200:
                return SourceOutcome.failure(failed_code='source_cdp_unavailable', safe_log=f'cdp_http_status={response.status_code}')
            payload = response.json()
            if not isinstance(payload, dict) or not payload.get('Browser'):
                return SourceOutcome.failure(failed_code='source_cdp_unavailable', safe_log='cdp_response_invalid')
        except (boss.requests.ConnectionError, boss.requests.Timeout):
            return SourceOutcome.failure(failed_code='source_cdp_unavailable', safe_log='cdp_port_unavailable')
        ...
        if self.browser_account:
            ...
            if cached == 'not_logged_in':
                return SourceOutcome.failure(failed_code='source_login_required', safe_log='boss_login_required cache=hit')
            if cached == 'unknown':
    ...
    def fetch_list(self, plan_item: dict, *, on_page_completed: Callable[[dict], None] | None=None) -> SourceOutcome:
        ...
        if not isinstance(plan_item, dict):
            return SourceOutcome.failure(failed_code='source_invalid_output', safe_log='plan_item_not_dict')
        ...
        if not keyword or not expected_hash:
            return SourceOutcome.failure(failed_code='source_invalid_output', safe_log=f'plan_item_missing_fields keyword={bool(keyword)} hash={bool(expected_hash)}')
        ...
        if not isinstance(raw_filters, dict):
            raw_filters = {}
        source_filters = {key: raw_filters[key] for key in SCRAPER_FILTER_FIELDS if raw_filters.get(key) not in (None, '', [])}
        output_path = plan_item.get('list_output_path') or plan_item.get('_list_output_path')
        if not output_path:
            return SourceOutcome.failure(failed_code='source_invalid_output', safe_log='plan_item_missing_list_output_path')
        if not self._artifact_path_allowed(output_path):
            return SourceOutcome.failure(failed_code='source_invalid_output', safe_log='list_output_path_invalid')
        ...
        def _drain_page_events():
            ...
            for line in lines[seen_page_events:]:
                ...
                if event.get('event_type') in {'scope_completed', 'explicit_empty'} or event.get('kind') in {'scope_completed', 'explicit_empty'}:
                    seen_page_events += 1
                    continue
                ...
                seen_page_events += 1
        command = self._build_list_command(keyword, city, target_pages, source_filters, str(output_path), combo_key=combo_key, list_events_output=page_events_path or None, start_page=start_page)
        ...
        if self.breaker.is_open() and (not self._try_breaker_recovery()):
            return SourceOutcome.failure(failed_code=self.breaker.open_failure_code(), safe_log=f'{safe_log} breaker_open')
        direct_page_events: list[dict] = []

        def _page_event(event):
            if isinstance(event, dict):
                direct_page_events.append(dict(event))
            if on_page_completed is not None:
                on_page_completed(event)
        try:
            returncode, captured = self._run_command(command, self.timeout_seconds, on_poll=_drain_page_events if not self.in_process else None, on_page_completed=_page_event)
        ...
        _drain_page_events()
        page_events = self._read_events_file(page_events_path) if page_events_path else []
        if direct_page_events:
            page_events.extend(direct_page_events)
        lost_empty_batch = not page_events
        if returncode != 0:
            failed_code = _classify_failed_code(returncode, captured)
            _logger.error('抓取子进程异常退出 stage=list returncode=%s failed_code=%s stderr_tail=%s', returncode, failed_code, _safe_tail(captured, max_chars=2000))
            self.breaker.record_signal(failed_code)
            reason = _exit_reason(returncode, captured)
            from webui import source as _facade
            _facade._record_risk_signals(self.browser_account, self.platform, failed_code, captured, run_id=self.run_id)
            return SourceOutcome.failure(failed_code=failed_code, safe_log=f'{safe_log} returncode={returncode} reason={reason}')
        ...
        if jobs is None:
            ...
            return SourceOutcome.failure(failed_code='source_invalid_output', safe_log=f'{safe_log} output_not_json')
        if not isinstance(jobs, list):
            return SourceOutcome.failure(failed_code='source_invalid_output', safe_log=f'{safe_log} output_not_list')
        actual_hash = _input_hash({'keyword': keyword, 'city': city, 'source_filters': source_filters, 'target_pages': target_pages})
        if expected_hash and actual_hash != expected_hash:
            return SourceOutcome.failure(failed_code='source_input_drift', safe_log=f'{safe_log} input_hash_mismatch expected_present={bool(expected_hash)}')
        normalized_jobs = [_normalize_job_fields(j) for j in jobs]
        scope_event = next((event for event in page_events if isinstance(event, dict) and (event.get('event_type') == 'scope_completed' or event.get('kind') == 'scope_completed')), None)
        ...
        return SourceOutcome.success(jobs=normalized_jobs, safe_log=f'{safe_log} job_count={len(normalized_jobs)}', input_hash=actual_hash, scope_complete=scope_event.get('scope_complete') if scope_event is not None else page_events[-1].get('scope_complete') if page_events else None, source_exhausted=scope_event.get('source_exhausted') if scope_event is not None else page_events[-1].get('has_more') is False if page_events else None, stop_reason=scope_event.get('stop_reason') if scope_event is not None else page_events[-1].get('stop_reason') if page_events else None, page_evidence=page_events)
    ...
    def fetch_detail(self, job: dict, *, detail_output_path: str | None=None) -> SourceOutcome:
        ...
        if not source_url or not job_id:
            return SourceOutcome.failure(failed_code='source_invalid_output', safe_log=f'job_missing_url_or_id job_id_present={bool(job_id)}')
        ...
        if not self._artifact_path_allowed(detail_path):
            return SourceOutcome.failure(failed_code='source_invalid_output', safe_log='detail_output_path_invalid')
        ...
        try:
            Path(detail_input_path).write_text(json.dumps({'jobs': [detail_job]}, ensure_ascii=False), encoding='utf-8')
        except OSError:
            return SourceOutcome.failure(failed_code='source_unreachable', safe_log='detail_input_write_failed')
        ...
        if returncode != 0:
            failed_code = _classify_failed_code(returncode, captured)
            _logger.error('抓取子进程异常退出 stage=detail returncode=%s failed_code=%s stderr_tail=%s', returncode, failed_code, _safe_tail(captured, max_chars=2000))
            self.breaker.record_signal(failed_code)
            from webui import source as _facade
            _facade._record_risk_signals(self.browser_account, self.platform, failed_code, captured, run_id=self.run_id)
            return SourceOutcome.failure(failed_code=failed_code, safe_log=f'{safe_log} returncode={returncode} stderr_tail_safe={_safe_tail(captured)}')
    ...
    _EVENT_FORBIDDEN_FIELDS = frozenset({'jd', 'jd_body', 'description', 'encrypt_job_id', 'encryptJobId', 'security_id', 'securityId', 'token', 'secret', 'api_key', 'apikey', 'prompt', 'model_response', 'raw_response', 'resume_text', 'resume_body', 'phone', 'email', 'id_card'})
    ...
    def _build_list_command(self, keyword: str, city: str, target_pages: int, source_filters: dict, output_path: str, combo_key: str='', list_events_output: str | None=None, start_page: int=1) -> list[str]:
        command = [self.python_executable, str(self.scraper_path), '--cdp-port', str(self.cdp_port), '--keyword', keyword, '--city', city or '_', '--pages', str(int(target_pages)), '--output', output_path, '--no-detail', '--skip-login-check']
    ...
    def _build_detail_command(self, detail_input_path: str, detail_output_path: str) -> list[str]:
        return [self.python_executable, str(self.scraper_path), '--cdp-port', str(self.cdp_port), '--input', detail_input_path, '--detail-output', detail_output_path, '--max-details', '1', '--detail']

    def _build_detail_batch_command(self, batch_input_path: str, detail_output_path: str, events_output_path: str, batch_size: int, gap_min: float=8, gap_max: float=15, reset_every: int=3, tab_pool_size: int=5, simulation_mode: str | None=None) -> list[str]:
        ...
        字节一致（既有命令断言不破坏）。
        """
        command = [self.python_executable, str(self.scraper_path), '--cdp-port', str(self.cdp_port), '--input', batch_input_path, '--detail-output', detail_output_path, '--events-output', events_output_path, '--max-details', str(batch_size), '--detail', '--enable-parallel', '--tab-pool-size', str(tab_pool_size), '--gap-min', str(gap_min), '--gap-max', str(gap_max), '--reset-every', str(reset_every)]
        if simulation_mode:
