    ...
    def fetch_list(
        ...
        try:
            ...
            def _page_event(event):
                if isinstance(event, dict):
                    page_evidence.append(dict(event))
                if on_page_completed is not None:
                    on_page_completed(event)
            runner_item["on_page_completed"] = _page_event
        ...
        if signal == "empty":
            ...
            return SourceOutcome.empty_success(
                ...
                input_hash=str(plan_item.get("input_hash") or ""),
                scope_complete=(page_evidence[-1].get("scope_complete") if page_evidence else None),
                source_exhausted=(page_evidence[-1].get("source_exhausted") if page_evidence else None),
                stop_reason=(page_evidence[-1].get("stop_reason") if page_evidence else None),
                page_evidence=page_evidence,
            )
