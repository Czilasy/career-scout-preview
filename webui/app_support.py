...
def build_app_support(app, store, runner, workbench_runner, job_feedback_service, history_service, resume_service, _prune_history_best_effort, _load_legacy_advanced_settings, _save_legacy_advanced_settings, _make_cdp_source, scope_previews, _runtime_mode, source_class):
    ...
    _tuning_round_runner = TuningRoundRunner(workspace_root=Path(store.db_path).resolve().parent.parent, source_factory=_make_cdp_source, ai_settings_provider=_tuning_ai_settings)
    ...
    def _new_pipeline_task(kind, *, source_task_id=None):
        task = {'kind': kind, 'status': 'queued', 'progress': {}, 'logs': [], 'result': None, 'error': '', 'started_at': int(time.time() * 1000), 'finished_at': None, 'stop_event': threading.Event()}
        if source_task_id:
            task['source_task_id'] = source_task_id
        return task

    def _register_pipeline_task(task_id, kind, *, source_task_id=None):
        task = _new_pipeline_task(kind, source_task_id=source_task_id)
        with _pipeline_lock:
    ...
    def _claim_recrawl_start(task_id, source_run_id):
        ...
        with _pipeline_lock:
            for existing_id, existing in _pipeline_tasks.items():
                if existing.get('kind') == 'recrawl' and existing.get('source_run_id') == source_run_id and (existing.get('status') in ('queued', 'running')):
    ...
    def _check_tuning_lease_conflict():
        ...
        if lease.get('owner_experiment_id') is None:
            return (True, None)
        return (False, (jsonify({'ok': False, 'error': 'tuning_lease_held', 'error_code': 'tuning_lease_held', 'message': '深度实验正在独占执行环境，请等待实验结束后再启动普通任务', 'owner_experiment_id': lease.get('owner_experiment_id'), 'retryable': False, 'required_action': '等待实验结束或取消实验后再启动普通任务'}), 409))
    ...
    def _is_user_finished(run_id):
        ...
        try:
            ...
            return False
        return bool(run and run.get('status') == 'interrupted' and (run.get('error_code') == 'user_finished'))
    ...
    def _record_pause_failure(task_id, stage, code, reason, *, processed=0, total=0, extra=None, exception=None, include_traceback=False):
        ...
        record_failure(store, task_id, stage=stage, error_code=code or 'internal_error', reason=reason or code or '任务被阻断', correlation_id=task_id, diagnostics=diagnostics, exception=exception, include_traceback=include_traceback)
    ...
    _SCREEN_STAGE_MESSAGES: dict[str, str] = {'resume': '正在恢复上次进度…', 'screen_a': 'AI 粗筛中…', 'screen_a_done': '粗筛完成，准备抓取 JD…', 'ensure_chrome': '启动浏览器，准备抓取 JD…', 'fetch_jd': '抓取 JD 中…', 'screen_b': 'AI 精筛中…', 'done': '筛选完成', 'cancelled': '运行已取消'}
    _EVENT_STAGE_NAMES = {'screen_a': 'ai_rough', 'screen_a_done': 'ai_rough', 'fetch_jd': 'jd_detail', 'screen_b': 'ai_fine'}
    ...
    def _ensure_scrape_source(scrape_task_id: str) -> dict | None:
        ...
        def _integrity(owner_id):
            try:
                return WhiteboxService(store).report('scrape', str(owner_id))['integrity']
            except Exception:
                return {'conclusion': 'unverifiable', 'label': '无法确认', 'evidence_complete': False, 'degraded': False, 'primary_code': 'legacy_evidence_missing', 'primary_reason': '历史证据不足，无法确认', 'recommendation': '建议重新执行', 'revision': 0}
        with _pipeline_lock:
            source_task = _pipeline_tasks.get(scrape_task_id)
            if source_task is not None:
                source_result = source_task.get('result') or {}
                integrity = source_result.get('integrity') or _integrity(scrape_task_id)
                if source_task.get('status') == 'done' and integrity.get('conclusion') in {'succeeded', 'empty'} and integrity.get('evidence_complete'):
                    return dict(source_task)
                if source_task.get('status') == 'done' and isinstance(source_result, dict):
                    legacy = dict(source_task)
                    legacy_result = dict(source_result)
                    legacy_result['integrity'] = integrity
        ...
        if source_run is None or source_run.get('status') not in ('succeeded', 'partial', 'failed', 'interrupted'):
            return None
        integrity = _integrity(scrape_task_id)
        snapshot = {'kind': 'scrape', 'status': 'done', 'progress': {}, 'logs': [], 'result': {'ok': integrity.get('conclusion') in {'succeeded', 'empty'} and integrity.get('evidence_complete'), 'jobs': source_jobs, 'total_scraped': len(source_jobs), 'total_matched': len(source_jobs), 'completed_combos': sorted(store.load_checkpoint(scrape_task_id, 'scrape')), 'error': '', 'integrity': integrity}, 'error': '', 'started_at': None, 'finished_at': None, 'stop_event': threading.Event(), 'auto_screen': bool((source_run.get('execution_params') or {}).get('auto_screen'))}
        with _pipeline_lock:
                ...
                if code in SYSTEMIC_BLOCK_CODES and code not in _ai_resume_codes:
                    ...
                    if not ai_service.is_ai_available(settings, credential_ref, api_key):
                        ...
                        if not capability.get('ok'):
                            passed = False
                            reason = taxonomy_reason(code, '', fallback='AI 阻断条件尚未解除')
    ...
    def _persist_jd_job_failures(task_run_id: str, jobs: list[dict], *, stage: str, source_run_id: str='', platform: str='') -> None:
        ...
        for job in jobs or []:
            ...
            events.append(('job_fail', {'stage': stage, 'job_id': job_id, 'failed_code': failed_code, 'reason': reason, 'evidence_detail': str(job.get('jd_failed_evidence') or '')}))
        if events:
