...
def match_jds(jobs_with_jd, profile_summary, endpoint_url, api_key, model="",
    ...
    system_prompt = (
        ...
        "判断规则：\n"
        "- 判断是参考不是法律：匹配从宽，宁可保留给用户人工确认，也不要因为类别、技能或行业不完全一致就排除。\n"
        ...
        "- JD 中'优先/加分/plus/熟悉/了解'类软性要求（如行业经验、英语等级、证书、技能未列出）不得影响 match，应写入 caveats 数组（每项一句话，如'优先英语六级，候选人未提供'）。\n"
        "- 行业、类别、技能不完全一致不排除。\n"
        "- 非开发类别（客服、讲师、销售、运营、内容/漫剧制作等）本身不得作为 match=false 的理由；用户画像未限定方向时更不能仅凭岗位类别排除。\n"
        "对每个岗位输出判定。严格输出JSON：\n"
