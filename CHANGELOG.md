...
- 重抓每批/暂停时向前端发布增量 updates，running/paused 轮询会实时合并，暂停后刷新展示来源 run 最新结果而不是旧快照。
- 新增结果快照计数重算，重抓解决待确认后 latest-pipeline-result 的匹配/不匹配/待确认/淘汰数量保持正确。

### 修复 — AI 续跑保留精筛结果，不再误报 RuntimeError
- 续跑时把精筛阶段已有的 not_match/uncertain 仍视为粗筛保留，不再只保留 match，避免结果缩水成仅匹配岗位。
- 修复精筛已完成时点“继续”出现“AI 筛选异常：RuntimeError”的问题；恢复后应正确收尾为完成但有待确认。

### 修复 — 暂停任务期间打开浏览器登录
- 暂停任务绑定的账号仍可“打开浏览器登录”，解决登录失效/验证码场景下必须先开浏览器才能恢复的死锁；其他账号仍锁定，并提示先结束或取消暂停任务。
- 前端错误提示优先显示后端返回的中文 message/user_message，不再把 `browser_busy` 等错误码直接展示给用户。

### 调整 — 浏览器账号添加表单
- 添加新浏览器账号的输入框改为较窄宽度，与“添加浏览器”按钮保持同一行，并移除输入框下方的说明文字。
...
- 完成任务后的计数按保留岗位统计，粗筛剔除不再显示成“未开始”，来源总数单独展示。
- AI 精筛开始时会立即把任务阶段切换为 ai_fine，页面不再在精筛期间继续显示“JD 详情”。
- 进入 AI 精筛时立即归零精筛进度，并把无 JD 岗位计为待确认，不再沿用 JD 阶段的成功计数。
- 已有更新的完成结果时，旧 paused/restart 中断任务不再抢占刷新后的恢复提示，页面直接展示最新结果。
...
- 刷新恢复重启中断任务时同步还原上次筛选条件与画像，重新开始会命中同一断点；旧历史 partial 快照不再覆盖中断提示。
- 服务重启中断的任务可直接“结束并保存结果”，无需先重新开始再等待暂停。
- 重启中断续跑会占用并标记旧 run 为已接管，重复提交同一来源返回 409；续跑完成后刷新不再回弹到旧中断记录。
- 恢复接口只认 error_code=restart，用户主动停止的任务不再被误判为服务重启中断；重启中断的抓取/补抓任务按真实阶段恢复。
- 续跑继承 JD 断点后，早期暂停也会把已继承 JD 落盘，避免二次恢复丢失已抓 JD；DB 重建抓取来源时校验源任务确实成功。
- 修复详情页误判：真实 JD 正文包含“限流/频繁/解锁”等岗位职责词时不再被当成账号限流页，只有拿不到真实 JD 的页面才做风控分类。
