...
```bash
# 基础搜索
python3 "$SCRIPT_PATH" --keyword "关键词" --city 城市 --pages 3 --output ~/.career-scout/job-result/jobs.json

# 带 CSV 输出
python3 "$SCRIPT_PATH" --keyword "关键词" --city 城市 --pages 3 --format csv --output ~/.career-scout/job-result/jobs.json

# 带详情 + 分析报告
python3 "$SCRIPT_PATH" --keyword "关键词" --city 城市 --pages 3 --detail --max-details 8 --analysis --format csv --output ~/.career-scout/job-result/jobs.json
...
# 合并多次抓取（去重）
python3 "$SCRIPT_PATH" --keyword "关键词" --city 北京 --pages 3 --merge ~/.career-scout/job-result/jobs.json --output ~/.career-scout/job-result/jobs_merged.json
```

默认输出到 `~/.career-scout/job-result/` 目录，`--format csv` 会给列表和详情都额外生成 `.csv` 文件。`--smoke-test` 只验证真实 Chrome/CDP 能否拿到 API 明文薪资，不写结果文件。
...
| `--output` | ~/.career-scout/job-result/... | 列表输出路径 |
| `--detail-output` | ~/.career-scout/job-result/... | 详情输出路径 |
...
`--setup-chrome` 默认使用持久隔离 profile，不软链接、不读取、不复制主 Chrome profile。首次启动和后续重复启动都只会创建或复用 `~/.career-scout/chrome-profile`，不会清空其中的 BOSS 登录态。setup 会等待登录完成，并用多组关键词/城市 probe，要求搜索接口返回明文薪资；如果一直拿不到 `salaryDesc`，不要继续抓取并把 DOM 薪资当成可信数据。这样 CDP 只暴露 BOSS 专用浏览器里的数据，不影响用户主 Chrome、Gmail、GitHub 等账号。
...
本 Skill 需手动安装到 Career Scout skills 目录（`hermes skills install` 因网络问题可能失败）：
...
mkdir -p ~/.hermes/skills/data-science/career-scout/scripts
cp career-scout/SKILL.md ~/.hermes/skills/data-science/career-scout/
cp career-scout/scripts/boss_cdp_raw.py ~/.hermes/skills/data-science/career-scout/scripts/
cp career-scout/scripts/job_summary.py ~/.hermes/skills/data-science/career-scout/scripts/
```
