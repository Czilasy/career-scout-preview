...
- T037 窗口状态文件：写入/读取、非法值回退默认、位置越界回退居中；目录可注入
...
class WindowStateTests(unittest.TestCase):
    ...
    def test_save_load_roundtrip(self):
        """写入后读取应返回相同值。"""
        desktop.save_window_state(1280, 800, 200, 150, state_dir=self.state_dir)
        result = desktop.load_window_state(state_dir=self.state_dir)
        self.assertEqual(result, (1280, 800, 200, 150))

    def test_load_missing_file_falls_back_default(self):
        """文件不存在 → 默认值。"""
        result = desktop.load_window_state(state_dir=self.state_dir)
        self.assertEqual(
    ...
    def test_load_invalid_json_falls_back_default(self):
        """损坏 JSON → 默认值。"""
        (self.state_dir / desktop.WINDOW_STATE_FILENAME).write_text(
    ...
    def test_load_non_numeric_fields_fall_back_default(self):
        """字段非数字 → 默认值。"""
        path = self.state_dir / desktop.WINDOW_STATE_FILENAME
        path.write_text(
            json.dumps(
                {"schema": 1, "width": "wide", "height": 800, "x": 0, "y": 0}
            ),
            encoding="utf-8",
        )
        result = desktop.load_window_state(state_dir=self.state_dir)
    ...
    def test_load_size_below_min_falls_back_default(self):
        """尺寸小于 min_size → 默认值。"""
        path = self.state_dir / desktop.WINDOW_STATE_FILENAME
        path.write_text(
            json.dumps(
                {"schema": 1, "width": 800, "height": 600, "x": 0, "y": 0}
            ),
            encoding="utf-8",
        )
        result = desktop.load_window_state(state_dir=self.state_dir)
    ...
    def test_load_size_over_upper_bound_falls_back_default(self):
        """尺寸超上界 → 默认值。"""
        path = self.state_dir / desktop.WINDOW_STATE_FILENAME
        path.write_text(
            json.dumps(
                {"schema": 1, "width": 99999, "height": 800, "x": 0, "y": 0}
            ),
            encoding="utf-8",
        )
        result = desktop.load_window_state(state_dir=self.state_dir)
    ...
    def test_load_wrong_schema_falls_back_default(self):
        """schema 版本不匹配 → 默认值。"""
        path = self.state_dir / desktop.WINDOW_STATE_FILENAME
        path.write_text(
            json.dumps(
                {"schema": 999, "width": 1280, "height": 800, "x": 0, "y": 0}
            ),
            encoding="utf-8",
        )
        result = desktop.load_window_state(state_dir=self.state_dir)
    ...
    def test_load_position_outside_workarea_centers(self):
        ...
        # 居中：(1920-1280)/2=320, (1080-800)/2=140
        self.assertEqual((x, y), (320, 140))
    ...
    def test_state_dir_injectable_no_real_user_dir(self):
        ...
        desktop.save_window_state(1400, 900, 50, 50, state_dir=isolated)
...
class ShellOrchestrationTests(unittest.TestCase):
    ...
    def test_closing_saves_window_state(self):
        ...
        desktop.run_desktop_shell(deps)
        result = desktop.load_window_state(state_dir=state_dir)
        self.assertEqual(result, (1400, 900, 200, 150))
    ...
    def test_restored_window_state_applied(self):
        """有窗口状态文件时恢复尺寸与位置。"""
        state_dir = tempfile.mkdtemp()
        desktop.save_window_state(1400, 900, 200, 150, state_dir=state_dir)
        webview_mod = _FakeWebview()
        deps = _make_deps(
            webview_module=webview_mod,
            state_dir=state_dir,
        ...
        desktop.run_desktop_shell(deps)
        kwargs = webview_mod.create_window_calls[0]
        self.assertEqual(kwargs["width"], 1400)
        self.assertEqual(kwargs["height"], 900)
        self.assertEqual(kwargs["x"], 200)
        self.assertEqual(kwargs["y"], 150)
...
class ReadVersionTests(unittest.TestCase):
    ...
    def test_reads_version_from_pyproject(self):
        ...
        self.assertEqual(version, "1.7.11")
