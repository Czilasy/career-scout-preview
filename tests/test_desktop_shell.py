...
class ShellOrchestrationTests(unittest.TestCase):
    ...
    def test_window_min_size_is_1024_700(self):
        """min_size = (1024, 700)（036 v2 常规下限；默认尺寸仍为 1400×800）。"""
        webview_mod = _FakeWebview()
...
class DesktopCloseTests(unittest.TestCase):
    """045 B102：桌面关闭必须按未完成流程决定保存/取消。"""

    def _make_host(self, *, latest=None, finish=None):
        class _Host:
            def __init__(self):
                self.latest_calls = 0
                self.finish_calls = []
            ...
            def latest_running_task(self):
                self.latest_calls += 1
                if callable(latest):
                    return latest(self.latest_calls)
                return latest

            def finish_run(self, run_id):
                self.finish_calls.append(run_id)
                if callable(finish):
    ...
    def test_no_task_closes_without_confirm(self):
        host = self._make_host(latest={"has_task": False})
        confirmed = []
        self.assertTrue(desktop.decide_close(host, confirm=lambda: confirmed.append(1)))
        self.assertEqual(host.latest_calls, 1)
        self.assertEqual(confirmed, [])
        self.assertTrue(host.state_saved)

    def test_zero_job_task_closes_silently(self):
        host = self._make_host(latest={"has_task": True, "task_id": "r", "job_count": 0})
        self.assertTrue(desktop.decide_close(host, confirm=self.fail))
        self.assertTrue(host.state_saved)

    def test_confirm_closes_after_saved_round(self):
        host = self._make_host(
            latest={"has_task": True, "task_id": "run-1", "job_count": 2},
            finish={"ok": True, "snapshot_run_id": "round-1"},
        )
        self.assertTrue(desktop.decide_close(host, confirm=lambda: True))
        self.assertEqual(host.finish_calls, ["run-1"])
        self.assertTrue(host.state_saved)

    def test_confirm_cancel_keeps_window(self):
        host = self._make_host(latest={"has_task": True, "task_id": "run-1", "job_count": 2})
        self.assertFalse(desktop.decide_close(host, confirm=lambda: False))
        self.assertEqual(host.finish_calls, [])
        self.assertFalse(host.state_saved)
    ...
    def test_finish_failure_keeps_window(self):
        ...
        self.assertFalse(desktop.decide_close(host, confirm=lambda: True))
        self.assertFalse(host.state_saved)

    def test_already_saved_with_round_closes(self):
        host = self._make_host(
            latest={"has_task": True, "task_id": "run-1", "job_count": 2},
            finish={"ok": False, "error": "already_finished"},
        )
        self.assertTrue(desktop.decide_close(host, confirm=lambda: True))
        self.assertTrue(host.state_saved)

    def test_latest_query_failure_keeps_window(self):
        def latest(_):
            raise RuntimeError("query failed")
        host = self._make_host(latest=latest)
        self.assertFalse(desktop.decide_close(host, confirm=self.fail))
        self.assertFalse(host.state_saved)
...
class ReadVersionTests(unittest.TestCase):
    ...
    def test_reads_version_from_pyproject(self):
        ...
        self.assertEqual(version, "1.9.4")
