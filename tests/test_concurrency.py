...
class CreateAnalysisConcurrentTests(_ConcurrentStoreFixture):
    """T028: create_analysis 并发建同一 resume 的 analysis。"""

    def test_two_threads_create_analysis_version_no_integrity_error(self):
        errors: list[Exception] = []
        barrier = threading.Barrier(2)
        results: list[dict] = []
        lock = threading.Lock()
        ...
        def worker():
            try:
                barrier.wait()
                a = self.store.create_analysis(self.resume["id"], self.profile["id"])
                with lock:
                    results.append(a)
            except Exception as exc:
                errors.append(exc)
        ...
        self.assertEqual(errors, [], f"create_analysis 并发抛错: {errors}")
        self.assertEqual(len(results), 2, "应成功创建 2 个 analysis")
        versions = sorted(a["version"] for a in results)
        self.assertEqual(versions, [1, 2], f"version 应为 1 和 2，实际 {versions}")


class CreateConfirmationConcurrentTests(_ConcurrentStoreFixture):
    """T029: create_confirmation（旧版）并发建同一 profile 的 confirmation。"""

    def test_two_threads_create_confirmation_version_no_integrity_error(self):
        # 需要先建 analysis + direction 才能 create_confirmation
        a = self.store.create_analysis(self.resume["id"], self.profile["id"])
        ...
        directions = [{"direction_id": d["id"], "enabled": True,
        ...
        # 多轮并发，每轮一对线程同时建 confirmation
        # 累计 10 对（20 个 confirmation），version 应为 1-20 连续无重复
        NUM_ROUNDS = 10

        def worker(round_barrier: threading.Barrier):
            try:
                round_barrier.wait()
                c = self.store.create_confirmation(
                    ...
                    safe_limits={}, directions=directions,
                )
                with lock:
                    results.append(c)
            except Exception as exc:
        ...
        self.assertEqual(errors, [], f"create_confirmation 并发抛错: {errors}")
        self.assertEqual(len(results), NUM_ROUNDS * 2,
                         f"应成功创建 {NUM_ROUNDS * 2} 个 confirmation，实际 {len(results)}")
        versions = sorted(c["version"] for c in results)
        expected = list(range(1, NUM_ROUNDS * 2 + 1))
        self.assertEqual(versions, expected,
                         f"version 应为 1-{NUM_ROUNDS * 2} 连续无重复，实际 {versions}")
